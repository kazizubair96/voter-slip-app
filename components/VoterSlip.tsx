...
const VoterSlip: React.FC<VoterSlipProps> = ({ voter }) => {
  ...
  const getCanvas = async () => {
    ...
    ctx.fillText('নাসিরনগর (২৪৩) | ত্রয়োদশ জাতীয় সংসদ নির্বাচন ২০২৬', 125, 105);
          ...
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-center md:items-start gap-3 md:gap-6 border-b border-slate-100 pb-3 md:pb-6 mb-4 md:mb-8">
            <div className="flex items-center gap-3 md:gap-4 text-center md:text-left">
              ...
              <div>
                ...
                <p className="text-[9px] md:text-sm font-bold text-[#006a4e] uppercase tracking-widest">নাসিরনগর (২৪৩) | সংসদ নির্বাচন ২০২৬</p>
              </div>
            </div>
            <div className="bg-red-600 text-white text-[7px] md:text-[10px] px-2.5 py-1 rounded-full font-black tracking-widest">CONFIDENTIAL</div>
          </div>

          <div className="relative z-10 grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-8 text-left">
            <div className="md:col-span-7 space-y-4 md:space-y-6">
              <div>
                <label className="text-[8px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-0.5">ভোটারের নাম</label>
                <p className="text-base md:text-2xl font-black text-slate-900 leading-tight">{voter.full_name}</p>
              </div>
              <div className="grid grid-cols-2 gap-3 md:gap-4">
                <div>
                  <label className="text-[8px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-0.5">পিতা/স্বামী</label>
                  <p className="text-[11px] md:text-base font-bold text-slate-700 truncate">{voter.father_name || '—'}</p>
                </div>
                <div>
                  <label className="text-[8px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-0.5">মাতার নাম</label>
                  <p className="text-[11px] md:text-base font-bold text-slate-700 truncate">{voter.mother_name || '—'}</p>
                </div>
              </div>
              <div className="pt-1 md:pt-2">
                <label className="text-[8px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-0.5">ভোট কেন্দ্র</label>
                <div className="flex items-start gap-1.5 md:gap-2">
                  <MapPin size={14} className="text-red-600 mt-1 shrink-0 md:w-[22px] md:h-[22px]" />
                  <p className="text-xs md:text-xl font-black text-slate-800 leading-tight">{voter.center_name}</p>
                </div>
              </div>
            </div>
            <div className="md:col-span-5 flex flex-col gap-3 md:gap-4">
              <div className="bg-slate-50 p-2.5 md:p-4 rounded-xl md:rounded-3xl border border-slate-100 flex items-center gap-2.5 md:gap-4">
                <div className="text-blue-600"><User size={16} className="md:w-6 md:h-6" /></div>
                <div>
                  <label className="text-[8px] md:text-[10px] font-bold text-slate-400 block uppercase">জন্ম তারিখ</label>
                  <span className="text-xs md:text-lg font-black text-slate-700">{voter.date_of_birth}</span>
                </div>
              </div>
              <div className="bg-[#006a4e] p-4 md:p-6 rounded-2xl md:rounded-[2rem] text-white shadow-xl flex flex-col justify-center">
                <label className="text-[8px] md:text-[10px] font-bold text-green-100 uppercase block mb-0.5">ভোটার নম্বর / সিরিয়াল</label>
                <p className="text-lg md:text-3xl font-black tracking-widest my-1 md:my-2 break-all">{voter.voter_number}</p>
                <div className="mt-2 md:mt-4 pt-2 md:pt-4 border-t border-white/10 flex justify-between items-center text-[9px] md:text-sm font-bold uppercase tracking-widest">
                  <span>ওয়ার্ড - {voter.ward}</span>
                  <CheckCircle size={14} className="md:w-[22px] md:h-[22px]" />
                </div>
              </div>
            </div>
          </div>
